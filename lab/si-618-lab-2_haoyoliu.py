...
def process_json():
  ...
  return sum(js[key] for key in js.keys())
...
def get_pigs():
  ...
  # the correct value.
  soup = BeautifulSoup(html_doc, 'html.parser')
  pigs = [i.h2.string for i in soup.findAll('li')]
  return json.dumps(pigs)
...
def get_houses():
  ...
  for i in soup.findAll('tr')[1:]:
    house = [i.findAll('td')[0].string, i.findAll('td')[1].string]
    houses.append(house)
...
def get_links():
  ...
  # the correct value.
  soup = BeautifulSoup(html_doc, 'html.parser')
  links = [i.get('href') for i in soup.findAll('a')]
...
def treasure_hunting():
  # Your code here: make sure to modify the return statement to pass back
  # the correct value.
  html_doc1 = urllib2.urlopen('http://www.example.com').read()
  soup = BeautifulSoup(html_doc1, 'html.parser')
  link = soup.a.get('href')

  html_doc2 = urllib2.urlopen(link).read()
  soup1 = BeautifulSoup(html_doc2, 'html.parser')
  result = soup1.table.findAll('tr')[3].td.string

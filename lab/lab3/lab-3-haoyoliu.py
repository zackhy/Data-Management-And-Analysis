...
with sqlite.connect(r'si-601-lab4.db') as con: 
  ...
  cur.execute("SELECT nation.N_NAME, Sum(customer.C_ACCTBAL) FROM nation, customer WHERE nation.N_NATIONKEY = customer.C_NATIONKEY GROUP BY nation.N_NAME ORDER BY nation.N_NAME")
  # fetchall() method gets all records. It returns a list of tuples.
  # Each of the tuples represent a row in the table.
  rows = cur.fetchall()
